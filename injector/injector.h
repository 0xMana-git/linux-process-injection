...
namespace Injector {
    class CInjector {
        constexpr static uint64 instructions_size = 24;
        constexpr static uint64 stack_size = 0x10000;
        pid_t pid;
        int mem_fd = -1;
        void* dst_syscall_addr = nullptr;
        //todo: restore 
        ...
        inline bool AllocateAnonymousAndWrite(const Buffer& buf, int prot = (PROT_READ | PROT_WRITE | PROT_EXEC), int added_flags = 0) {
            void* dst = AllocateAnonymous(buf.buf_size, prot, added_flags);
            return WriteBufferToProcess(buf, dst);
        }
        ...
        public:
        //parameter is an arbitrary param passed in rdx
